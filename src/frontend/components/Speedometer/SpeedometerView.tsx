    ...
    <svg width={375} height={375}>
      ...
      {Array.from({ length: totalMarks + 1 }).map((_, i) => {
        ...
        return (
          <g key={i}>
            {isVisible && (
              <line x1={markX1} y1={markY1} x2={markX2} y2={markY2} stroke={i % 2 === 0 ? "#FFF" : contourColor} strokeWidth={i % 2 === 0 ? 3 : 2} />
            )}
            {isVisible && i % 2 === 0 && (
              <text x={numberX} y={numberY} fill="#FFF" fontSize="20" fontWeight="bold" textAnchor="middle" alignmentBaseline="middle">
                {value}
              </text>
            )}
          </g>
      ...
      {/* Цифровой спидометр */}
      <text x={centerX} y={centerY - 10} fill="#FFF" fontSize="48" fontWeight="bold" textAnchor="middle" alignmentBaseline="middle">
        {displaySpeed}
      </text>

      {/* km/h */}
      <text x={centerX} y={centerY + 30} fill="#FFF" fontSize="24" fontWeight="bold" textAnchor="middle" alignmentBaseline="middle">
        km/h
      </text>
    </svg>
  );

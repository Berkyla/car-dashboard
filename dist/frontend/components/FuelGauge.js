...
const FuelGauge = ({ fuelLevel, maxFuelLevel }) => {
    const width = 300;
    const height = 300;
    ...
    // Угол стрелки
    const fuelAngle = Math.PI / 3 - normalizedFuelLevel * ((2 * Math.PI) / 3);
    ...
    return (React.createElement("svg", { width: width, height: height },
        ...
        [0, 0.5, 1].map((value, i) => {
            ...
            return (React.createElement("g", { key: `fuel-tick-${i}` },
                ...
                React.createElement("text", { x: textX, y: textY, fill: "#fff", fontSize: "16px", fontWeight: "bold", textAnchor: "middle", dominantBaseline: "middle" }, value)));
        }),
        [0.25, 0.75].map((value, i) => {
            ...
            return (React.createElement("line", { key: `intermediate-tick-${i}`, x1: x1, y1: y1, x2: x2, y2: y2, stroke: "#555" // Цвет как у контура
                , strokeWidth: 2 }));
        }),
